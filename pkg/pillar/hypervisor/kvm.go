...
const igdAddress = "00:02.0"

const pcMachine = "pc-i440fx-5.1"
...
{{- if eq .PCIType "pcie"}}
  bus = "pcie.0"
  addr = "{{printf "0x%x" .PCIIdGraphics}}"
{{- else}}
  bus = "pci.0"
  addr = "{{printf "0x%x" .PCIIdGraphics}}"
{{- end}}
...
{{- if .VGARom }}
  x-igd-gms = "2"
  x-igd-opregion = "on"
  romfile = "/usr/lib/xen/boot/IgdAssignmentDxe.rom"
{{- end -}}
`
...
func (ctx kvmContext) CreateDomConfig(domainName string, config types.DomainConfig, diskStatusList []types.DiskStatus,
	...
	for _, adapter := range config.IoAdapterList {
		...
		for _, ib := range list {
			...
			if ib.PciLong != "" {
				...
				if types.PCILongToShort(ib.PciLong) == igdAddress {
					hasIGD = true
				}
			}
			if ib.Serial != "" {
	...
	tmplCtx := struct {
		...
		types.DomainConfig
		PCIType                 string
		PCIIdUSB, PCIIdGraphics int
	}{Machine: ctx.devicemodel, DomainConfig: config, PCIType: currentPCIType, PCIIdGraphics: 1, PCIIdUSB: 2}
	tmplCtx.Memory = (config.Memory + 1023) / 1024
	tmplCtx.DisplayName = domainName
	if hasIGD {
		tmplCtx.Machine = pcMachine
		// slot 1 for PIIX3
		// add space in 0x2 for IGD
		tmplCtx.PCIIdUSB = 4
		tmplCtx.PCIIdGraphics = 5
	}
	...
	// render global pci device settings
	t, _ = template.New("qemuPci").Parse(qemuPciCommon)
	if err := t.Execute(file, tmplCtx); err != nil {
		return logError("can't write to config file %s (%v)", file.Name(), err)
	}
	...
	diskContext := struct {
		...
		PCIType               string
		types.DiskStatus
	}{Machine: ctx.devicemodel, PCIId: 4, DiskID: 0, SATAId: 0, AioType: "io_uring", PCIType: currentPCIType}
	if hasIGD {
		diskContext.PCIId = 6
	}
	...
	netContext := struct {
		...
		PCIType          string
	}{PCIId: diskContext.PCIId, NetID: 0, PCIType: currentPCIType}
	...
	if len(pciAssignments) != 0 {
		pciPTContext := struct {
			...
			PCIType      string
			VGARom       bool
		}{PCIId: netContext.PCIId, PciShortAddr: "", Xvga: false, PCIType: currentPCIType, VGARom: false}
		...
		for _, pa := range pciAssignments {
			...
			if _, err := os.Stat(bootVgaFile); err == nil {
				...
				_ = ctx.unbindFramebuffer()
			}
			currentPCIId := pciPTContext.PCIId
			if short == igdAddress {
				pciPTContext.PCIId = 2
				pciPTContext.VGARom = true
				currentPCIId--
			}
			...
			pciPTContext.PCIId = currentPCIId + 1
		}
	}
...
func (ctx kvmContext) unbindFramebuffer() error {
	logrus.Debug("UnbindFramebuffer")

	unbindFile := "/sys/bus/platform/drivers/efi-framebuffer/unbind"

	//Unbind framebuffer, if unbind file is present
	if _, err := os.Stat(unbindFile); err == nil {
		if err := ioutil.WriteFile(unbindFile, []byte("efi-framebuffer.0"), 0644); err != nil {
			return logError("unbind failure for efi-framebuffer: %v", err)
		}
	}
