...
func (hdl metadataHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	log.Tracef("metadataHandler ServeHTTP request: %s", r.URL.String())
	...
	if anConfig.MetaDataType != types.MetaDataEC2 {
		errorLine := fmt.Sprintf("no MetaDataEC2 for %s",
			anStatus.Key())
...
// ServeHTTP for userDataHandler process requests for EC2 user-data
func (hdl userDataHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	log.Tracef("userDataHandler ServeHTTP request: %s", r.URL.String())
	remoteIP := net.ParseIP(strings.Split(r.RemoteAddr, ":")[0])
	anStatus := lookupAppNetworkStatusByAppIP(hdl.ctx, remoteIP)
	if anStatus != nil {
		anConfig := lookupAppNetworkConfig(hdl.ctx, anStatus.Key())
