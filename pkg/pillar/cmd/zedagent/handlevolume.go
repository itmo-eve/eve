...
func parseVolumeConfig(ctx *getconfigContext,
	...
	for idStr, vc := range items {
		...
		if !found {
			...
			volumeConfig.Stale = checkStaleVolume(ctx, cfgVolume, config)
...
// checkStaleVolume returns true if there are no apps using this image in new config
func checkStaleVolume(ctx *getconfigContext, cfgVolume *zconfig.Volume, devConfig *zconfig.EdgeDevConfig) bool {
